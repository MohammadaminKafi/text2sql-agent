...
def compare_dataframes_as_dataframe_safe(
    gt_df: pd.DataFrame, out_df: pd.DataFrame
) -> pd.DataFrame:
    result_dict = {
        "Metric": [
            "gt_rows",
    ...
    try:
        gt_not_in_out = len(
            pd.merge(gt_df, out_df, how="left", indicator=True).query(
                '_merge == "left_only"'
            )
        )
    except Exception:
    ...
    try:
        out_not_in_gt = len(
            pd.merge(out_df, gt_df, how="left", indicator=True).query(
                '_merge == "left_only"'
            )
        )
    except Exception:
    ...
    try:
        gt_in_out = (
            gt_df.shape[0] <= out_df.shape[0]
            and gt_df.columns.isin(out_df.columns).all()
            and gt_df.equals(out_df.iloc[: gt_df.shape[0], :])
        )
    except Exception:
        gt_in_out = None
    ...
    try:
        out_in_gt = (
            out_df.shape[0] <= gt_df.shape[0]
            and out_df.columns.isin(gt_df.columns).all()
            and out_df.equals(gt_df.iloc[: out_df.shape[0], :])
        )
    except Exception:
        out_in_gt = None
...
def generate_final_report(model_dir: Path) -> None:
    ...
    with open(final_path, "w", newline="", encoding="utf-8") as f:
        ...
        for cat_dir in sorted(p for p in model_dir.iterdir() if p.is_dir()):
            ...
            writer.writerow(
                [cat_dir.name, len(df)]
                + [f"{m:.3f}" if m == m else "0.000" for m in metrics]
            )
